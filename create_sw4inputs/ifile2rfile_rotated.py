...
outpath = '/Users/oluwaseunfadugba/Documents/Projects/TsE_ValerieDiego/TsE_1D_vs_3D/'+\
    '3D_Modeling_using_SW4/2_Creating_Rfile/Creating_rfile_5nblks_z200_h500_rotated_35deg/'
os.chdir(outpath)
...
if az!=0:
    ...
    xdim = 871; ydim = 2101 #720

    w = 0;  e = (xdim-1)*1e3 #720e3
    s = 0;  n = (ydim-1)*1e3

rfileoutput = '3djapan_hv=2_3_4_500m_rot35.rfile'

# Creating an array for the velocity model
ifile_in = 'ALL_japan_path_large.dat'
n_tif = 23
dx = 1000 #(m)
dy = -1000 #(m)

#proj4 = ('+proj=tmerc +datum=NAD83 +units=m +lon_0=126.2932 +lat_0=31.8341 +no_defs')
proj4 = ('+proj=tmerc +datum=NAD83 +units=m +lon_0='+str(lon_orig)+' +lat_0='+str(lat_orig)+' +no_defs')
...
if az !=0:
    ...
    tiffs_orig=glob(outpath+'/tiff/elev_layer*.tif')
    
    os.system('mv tiff tiff_norot')
    os.system('mkdir tiff')
    
    tiffs=glob(outpath+'/tiff_norot/elev_layer*.tif')
    
    for i in range(len(tiffs)):
        j = i+1
        
        Original_Image = Image.open(tiffs[i])
     
        # This Will Rotate Image By 35 Degree
        rotated_image = Original_Image.rotate(az,expand=1)
         
        x = x0; y = y0
        box = (x, y, x+xdim, y+ydim) # (left, upper, right, lower)
        rotated_cropped_image = rotated_image.crop(box)
        
        tiff_data_i = np.array(rotated_cropped_image.getdata()).reshape(ydim,xdim)
        
        # save rotated file to tiff
        #rotated_cropped_image.save(tiffs_orig[i],'tiff'); 
        
        # Creating TIFF for each rotated layer
        tlx = 0
        tly = n-s #1000
 
        geo.save_GeoTIFF(tiffs_orig[i], tiff_data_i, 
                          tlx, tly, dx, dy, epsg=None, proj4=proj4, 
                          dtype=np.float32, nodata=np.nan, rasterBand=1)

        # plt.imshow(rotated_image, cmap='terrain'); 
        # rotated_image.save(tiffs[i][:-3]+'rot_intm.tif','tiff'); 
        # plt.show()
  
        #plt.imshow(rotated_cropped_image, cmap='terrain'); 
        #plt.show()
...
az           = np.float64(az)
lon0         = np.float64(lon_orig) 
lat0         = np.float64(lat_orig) 
            ...
            for j in range(nj_blk):
                ...
                if j == 0 and i%100 == 0:
                    print('\rWriting row {} of {}, '
                          'column {} of {} of block {}...'.format(i + 1, ni_blk,
                                                                  j + 1, nj_blk,
                                                                  blk+1)),
                flush
...
cs = model.get_cross_section(0, 2040, 720,720)
fig, ax, cb = cs.plot(property='vp', vmin=1800,aspect=10, cmap='jet') #vmin=0,
figpath = os.getcwd() +'/fig.crossection1.png'
plt.savefig(figpath, bbox_inches='tight', dpi=200) 
plt.show()
...
cs = model.get_cross_section(1000, 1000, 0, 720)
...
# z, properties = model.get_z_profile(100, 100)
